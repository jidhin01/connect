...
import {
  ArrowLeftIcon, EllipsisVerticalIcon, PhoneIcon, VideoCameraIcon,
  PaperClipIcon, FaceSmileIcon, MicrophoneIcon, PhotoIcon,
  PaperAirplaneIcon, CheckIcon, CheckBadgeIcon, XMarkIcon, BookmarkIcon
...
export default function Chat() {
  ...
  const conversationId = typeof window !== "undefined" ? localStorage.getItem("activeConversationId") : null;
  const conversationName = typeof window !== "undefined" ? localStorage.getItem("activeConversationName") : null; // the other user's name
  ...
  // If not ready, show a goto button
  if (!token || !conversationId || !conversationName) {
    return (
      <div className="h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="mb-3 text-sm text-red-600">
            Missing token or conversation. <br />Please select a user from Contacts.
          </p>
          <button
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const optimistic = {
        id: tempId,
        author: { id: myId, name: "Me", avatar: "https://i.pravatar.cc/80?img=5" },
        text,
  ...
  return (
    <div className="h-screen bg-seco text-gray-900 flex flex-col">
      {/* Header */}
      <header className="sticky top-0 z-20 bg-white rounded-3xl border-b border-gray-200">
        <div className="mx-auto max-w-4xl px-4">
          <div className="h-16 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <img src={partner.avatar} alt={partner.name} className="h-10 w-10 rounded-full" />
              <div>
        ...
        <div className="mx-auto max-w-4xl px-4 pb-3">
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-indigo-50 border border-indigo-100">
            <BookmarkIcon className="h-5 w-5 text-btn" />
            <p className="text-sm text-indigo-900 truncate">
              Pinned: Release checklist doc → Remember to update screenshots.
            </p>
            <button className="ml-auto p-1 rounded hover:bg-indigo-100">
              <XMarkIcon className="h-4 w-4 text-indigo-700" />
            </button>
          </div>
        </div>
      </header>
      ...
      {/* Message list */}
      <main className="mx-auto max-w-4xl w-full px-4 flex-1 overflow-y-auto py-4 no-scrollbar" ref={listRef}>
        <DayDivider label="Today" />

        {loading && <div className="text-sm text-gray-600 py-2">Loading…</div>}
        {!!err && !loading && <div className="text-sm text-red-600 py-2">Error: {err}</div>}
      ...
      <footer className="bg-white border-t rounded-t-3xl border-gray-200">
...
function MessageBubble({ msg, showAvatar }) {
  const base = "max-w-[80%] md:max-w-[70%] px-3 py-2 rounded-2xl text-sm leading-relaxed shadow-sm";
  const bubble = msg.outgoing
    ? "bg-btn text-white rounded-br-md ml-auto"
      ...
      <div className="min-w-0">
        <div className={`${base} ${bubble}`}>
          {!msg.outgoing && showAvatar && (
            <p className="text-[11px] font-medium text-gray-500 mb-1">{msg.author.name}</p>
          )}
          <p className="break-words">{msg.text}</p>
          <div className={`mt-1 flex items-center gap-1 ${msg.outgoing ? "justify-end" : "justify-start"}`}>
            <span className={`text-[11px] ${msg.outgoing ? "text-indigo-100" : "text-gray-500"}`}>
              {msg.time}
            </span>
